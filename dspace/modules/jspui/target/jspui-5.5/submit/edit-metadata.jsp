...
%>
<%!
    //this will be used from COMPASS ASSOCIATION ENGINE.
    CompassBasicPopulator compassHelper = new CompassBasicPopulator();
    CompassLoInstanceFormPopulator compassLoInstanceFormPopulator = new CompassLoInstanceFormPopulator();
    ...
    /**
        MODIFICATION IN ORDER TO WORK FOR: COMPASS ASSOCIATION ENGINE.
        @author Anastasios Tsolakidis.
        @author Nikolaos Christidis.
    */
    void doTextArea(javax.servlet.jsp.JspWriter out,
                    Item item,
                    String fieldName, 
                    String schema, String element, String qualifier, 
                    boolean repeatable, boolean required, boolean readonly,
                    int fieldCountIncr,
                    String label, PageContext pageContext, 
                    String vocabulary, boolean closedVocabulary, int collectionID,
                            int pageNumber, Context context)
            throws java.io.IOException {


            
        //================START OF MODIFICATION=======================================================
        Metadatum[] defaults = null;
        try{

            //the key functionality is to hold the same item, or use the lospec item...
//            out.println("before great escape");
            
            Item tempItem = compassLoInstanceFormPopulator
                    .getSelectedLOSpecificationFromSpecifierAttributeOfLOInstance(label, pageNumber, item, context);
            
            if (tempItem != null) {
//                out.println("great escape worked!");
                
                LOInstanceFormAction lOInstanceFormAction = LOInstanceFormDb.getFieldExtractor(label);

                if (lOInstanceFormAction != null) {
                    defaults = lOInstanceFormAction.extractCorrectInfoFromLOSpec(tempItem);
                }
                           
            } else {
                defaults = item.getMetadata(schema, element, qualifier, Item.ANY);
            }
            
//            out.println("great escape not worked");

        } catch(Exception ex) {
            out.println("COMPASS ENGINE ERROR OCCURRED: " + ex.getMessage());
        }

        if (defaults == null) {
            defaults = new Metadatum[0];
        }
        //================END OF MODIFICATION=========================================================
    ...
    /**
        ...
        @author Nikolaos Christidis.
    */
    void doOneBox(javax.servlet.jsp.JspWriter out, 
            Item item,
            String fieldName, 
            String schema, String element, String qualifier, 
            boolean repeatable, boolean required, boolean readonly,
            int fieldCountIncr, 
            String label, PageContext pageContext, String vocabulary, boolean closedVocabulary, int collectionID,
            int pageNumber, Context context)
            throws java.io.IOException {
...
//            out.println("great escape not worked");
    ...
    /**
     * THIS METHOD IS MODIFIED IN ORDER TO SUPPORT COMPASS ASSOCIATION BETWEEN
    ...
    void doDropDown(javax.servlet.jsp.JspWriter out, Item item,
            ...
            List valueList, String label, int pageNumber, Collection collection, String collectionHandle, Item itemToBeSubmitted)
            throws java.io.IOException {
        try {
            out.println("valueList = " + valueList.size());

            List tempList = compassHelper.modify_doDropDown_valueList_jspui_method(pageNumber, collection,
                    collectionHandle, itemToBeSubmitted);
            ...
            out.println("###COMPASS ASSOCIATION ENGINE ERROR = " + ex.getMessage() + " ###");
        }
