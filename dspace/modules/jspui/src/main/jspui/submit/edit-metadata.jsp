...
<%@page import="org.dspace.compass.CompassHelper"%>
<%@page import="org.dspace.compass.Tupple2"%>
...
//            .append("_add\" value=\"Add More\"/> </td></tr>");
    ...
    /**
     * THIS METHOD IS MODIFIED IN ORDER TO SUPPORT COMPASS ASSOCIATION BETWEEN ITEMS FROM THE SAME COLLECTION.
     * @author Anastasios Tsolakidis.
     * @author Nikolaos Christidis.
     */
    void doDropDown(javax.servlet.jsp.JspWriter out, Item item,
      String fieldName, String schema, String element, String qualifier, boolean repeatable,
      boolean required, boolean readonly, List valueList, String label, int pageNumber, Collection collection, String collectionHandle)
      throws java.io.IOException
    {
        try{
            out.println("valueList = " + valueList.size());
            
            List l = compassHelper.modify_doDropDown_valueList_jspui_method(pageNumber, collection, collectionHandle);

            if (l.isEmpty()) {
                out.println("l is empty");
            } else {
                out.println("l is not empty");
                valueList = l;
            }
       ...
       else if (inputType.equals("dropdown"))
       {
           ...
           //}
                        ...
                        doDropDown(out, item, fieldName, dcSchema, dcElement, dcQualifier,
                                   repeatable, required, readonly, inputs[z].getPairs(), label, pageNum, collection, collectionHandle);
       }
